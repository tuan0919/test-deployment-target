pipeline {
  ...
  stages {
    ...
    stage('Pre-deploy Kopia Snapshot') { when { expression { !params.RUN_ROLLBACK } } steps { sshagent(credentials: ['DEPLOY_SSH_KEY']) { script { env.PREVIOUS_IMAGE = sh(script: 'ssh $DEPLOY_USER@$VM_IP "cat /srv/eac-demo/state/current-image 2>/dev/null || true"', returnStdout: true).trim() } withCredentials([usernamePassword(credentialsId: 'KOPIA_CREDENTIALS', usernameVariable: 'KOPIA_USERNAME', passwordVariable: 'KOPIA_PASSWORD')]) { sh 'set +x; ssh $DEPLOY_USER@$VM_IP "KOPIA_SERVER=\"$KOPIA_SERVER\" KOPIA_USERNAME=\"$KOPIA_USERNAME\" KOPIA_PASSWORD=\"$KOPIA_PASSWORD\" bash -s" < scripts/backup.sh | tee snapshot.log' } script { env.SNAPSHOT_LOG = readFile('snapshot.log').trim() } } } }
    stage('Deploy') { when { expression { !params.RUN_ROLLBACK } } steps { sshagent(credentials: ['DEPLOY_SSH_KEY']) { sh 'ssh $DEPLOY_USER@$VM_IP "bash -s -- \"$IMAGE_REF\"" < scripts/deploy.sh' } } }
    stage('Health Check') { when { expression { !params.RUN_ROLLBACK } } steps { sshagent(credentials: ['DEPLOY_SSH_KEY']) { sh 'ssh $DEPLOY_USER@$VM_IP "bash -s -- \"${IMAGE_REF##*:}\"" < scripts/health-check.sh' } } }
